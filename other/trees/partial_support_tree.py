...
class PartialSupportTree:
            ...
            if self.children is not None:
                for child in self.children:
                    if child.item_name == item_name:
                        ...
                        return
                new_child = PartialSupportTree.Node(item_name, frequency, self)
    ...
    def create_tree(item_sets, min_support_ratio, scaling_factor):
        ...
        supports = PartialSupportTree.get_support_nic(
            item_sets, min_support, scaling_factor)

        tree = PartialSupportTree.Node('Null', 1)
        for row in item_sets:
            item_set = [
    ...
    def prefix_trees(tree):
        ...
        if tree.children is not None:
            for idx in range(len(tree.children)):
                new_tree = PartialSupportTree.Node('Null', 1)
