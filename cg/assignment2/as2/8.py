...
def translate_dda_line(canvas, starting, ending, factors):
    ...
    dda_line_drawing(
        canvas, starting[0], starting[1], ending[0], ending[1], color=colors.secondary)
    ...
    dda_line_drawing(canvas, new_starting[0], new_starting[1],
                     new_ending[0], new_ending[1], color=colors.primary)
...
if __name__ == "__main__":
    ...
    window.set_input(
        ["Starting point(x1,y1)", "Ending point (x2,y2)", "Translation factor(tx,ty)"])
    window.run()
