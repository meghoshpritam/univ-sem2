...
def put_pixel(canvas, x, y, outline=None, width=5):
    x = int(x)
    y = int(y)
    ...
    canvas.create_oval(x, y, x, y, width=width,
                       fill=colors.white, outline=outline)

if __name__ == "__main__":
    window = Window(put_pixel)
    window.set_title("Draw a pixel on screen")
    window.set_input(["Enter x:", "Enter y:", "Enter Color:", "Enter Width:"])
