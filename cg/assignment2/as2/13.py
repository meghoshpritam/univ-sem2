...
def transform(canvas, start, end, angle, factor, scaling_factor):
    color = Utils()
    start = Helper.string_to_coordinates(start)
    end = Helper.string_to_coordinates(end)
    angle = int(angle)
    factor = Helper.string_to_coordinates(factor)
    scaling_factor = Helper.string_to_coordinates(scaling_factor)

    canvas.create_line(*[*start, *end], fill=color.secondary, width=5)

    new_rotate = Transformation.rotate([*start, *end], angle)
    new_translate = Transformation.translate([*start, *end], factor)
    new_scaling = Transformation.scaling([*start, *end], scaling_factor)

    canvas.create_line(*new_rotate, fill="yellow", width=5)
    canvas.create_line(*new_translate, fill=color.primary, width=5)
    canvas.create_line(*new_scaling, fill="green", width=5)


if __name__ == "__main__":
    window = Window(transform)
    window.set_title("Basic Transformations on a line")
    window.set_input(["Starting point(x1,y1)",
                      "Ending point(x2,y2)", "Angle of rotation", "Translation factor(x,y)", "Scaling factor(Sfx,Sfy)"])
    window.run()
